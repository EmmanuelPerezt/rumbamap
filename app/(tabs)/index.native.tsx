...
import { DEFAULT_LOCATION, events, mapMarkers } from '@/data/mock';
...
function CustomMarkerView({ type }: { type: MapMarkerType['type'] }) {
  const color = type === 'match' ? PINK : type === 'hospitality' ? ORANGE : BLUE;
  return (
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Use default location — no action needed
      }
    })();
  }, []);
